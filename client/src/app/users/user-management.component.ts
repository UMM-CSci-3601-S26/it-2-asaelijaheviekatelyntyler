...
export class UserManagementComponent implements OnInit {
  ...
  createJobRole() {
    const name = this.newJobRoleName.trim();
    if (!name) {
      return;
    }
  ...
  deleteJobRole(name: string) {
    ...
    if (!confirm(`Delete job role ${name}?`)) {
      return;
    }
  ...
  private buildUserPayload(): UserUpsertRequest {
    ...
    return {
      ...
      jobRole: raw.systemRole === 'VOLUNTEER' ? (raw.jobRole?.trim() || 'volunteer_base') : null
    };
  }
  ...
  private buildPermissionGroups(permissionCatalog: PermissionCatalogEntry[]) {
    const groupOrder = ['Family', 'Inventory', 'Supply List', 'Checklist', 'Settings'];
