 ...
 * - POST /api/auth/signup → create new user, authenticate, and issue JWT cookie
 * - POST /api/auth/logout → clear authentication cookie
 * - GET /api/auth/me → return current user's access profile based on JWT cookie
 * - GET /api/auth/permissions → return current user's access profile based on JWT cookie
 * - GET /api/auth/permissions/all → (admin only) return all system and job roles with their permissions
 * - GET /api/auth/job-roles → (admin only) return all job roles with their permissions
 * - PUT /api/auth/job-roles/{jobRole} → (admin only) create or update a job role's permissions
 * - DELETE /api/auth/job-roles/{jobRole} → (admin only) delete a job role
 * - PUT /api/auth/users/{username}/job-role → (admin only) assign a job role to a volunteer user
 */
...
public class AuthController {
  ...
  public void signup(Context ctx) {
    ...
    // If systemRole is provided and is "guardian" (case-insensitive), set system role to GUARDIAN, otherwise default to VOLUNTEER
    // This does not allow users to sign up directly as admins, and ensures that any invalid or unrecognized systemRole values will result in a volunteer account rather than causing an error
  ...
  private AccessIdentity normalizeAccessIdentity(Users user) {
    ...
    return new AccessIdentity(systemRole, jobRole);
  }
  ...
  private static class AccessIdentity {
    private final String jobRole;

    // Constructor to initialize the AccessIdentity with the provided job role
    private AccessIdentity(Role systemRole, String jobRole) {
